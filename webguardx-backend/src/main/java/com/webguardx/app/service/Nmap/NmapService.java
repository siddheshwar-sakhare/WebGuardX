...
public class NmapService {
    ...
    public NmapScanResult scan(NmapScanRequest request, User user) {
        ...
        try {
            ...
            if (!isNmapAvailable()) {
                logger.warn("Nmap not installed locally. Returning mock data for demonstration purposes.");
                return getMockScanResult(request.getTarget());
            }
    ...
    private boolean isNmapAvailable() {
        try {
            ...
            logger.warn("Nmap not available (this is expected in demo environments without Nmap installed).");
            return false;
        }
    }

    private NmapScanResult getMockScanResult(String target) {
        NmapScanResult mockResult = new NmapScanResult();
        mockResult.setTarget(target);
        mockResult.setStatus("SUCCESS");
        mockResult.setMessage("Demo Scan completed successfully");
        
        NmapHostInfo hostInfo = new NmapHostInfo();
        hostInfo.setStatus("up");
        mockResult.setHostInfo(hostInfo);

        NmapOsDetection osDetection = new NmapOsDetection();
        osDetection.setName("Microsoft Windows 11 Pro (Build 22621)");
        mockResult.setOsInfo(osDetection);
        
        List<NmapPort> openPortsList = new ArrayList<>();
        List<NmapPort> filteredPortsList = new ArrayList<>();
        
        NmapPort msrpc = new NmapPort();
        msrpc.setPort(135);
        msrpc.setProtocol("tcp");
        msrpc.setState("open");
        msrpc.setService("msrpc");
        msrpc.setVersion("Microsoft Windows RPC");
        openPortsList.add(msrpc);

        NmapPort http = new NmapPort();
        http.setPort(80);
        http.setProtocol("tcp");
        http.setState("open");
        http.setService("http");
        http.setVersion("nginx 1.18.0");
        openPortsList.add(http);
        
        NmapPort https = new NmapPort();
        https.setPort(443);
        https.setProtocol("tcp");
        https.setState("open");
        https.setService("https");
        https.setVersion("nginx 1.18.0");
        openPortsList.add(https);


        NmapPort rdp = new NmapPort();
        rdp.setPort(3389);
        rdp.setProtocol("tcp");
        rdp.setState("filtered");
        rdp.setService("ms-wbt-server");
        rdp.setVersion("N/A");
        filteredPortsList.add(rdp);

        mockResult.setOpenPorts(openPortsList);
        mockResult.setFilteredPorts(filteredPortsList);
